...
async function fetchToBlob(url, { key, durSec, noOpen = false, signal, onOpen, onFull, onProgress } = {}) {
...
export async function playBuffered(el, url, { rate = 1, key, dur, onStage, onFull } = {}) {
  ...
  /** Playback is about to outrun the download: hold at the seam, like any
   *  streaming player buffers, and give up on the pipe entirely if the
   *  rest does not arrive within half a minute. */
  const holdAtSeam = () => {
  ...
  const onError = () => {
    if (ses.closed || !partialUrl || el.error?.code !== 3) return;
    if (fullUrl) { doSwap(); return; }
  ...
  const onTimeUpdate = () => {
    if (ses.closed || swapped || fullUrl || !partialUrl || !playStarted) return;
    const ps = partialSec();
    if (ps != null && el.currentTime > ps - 6) holdAtSeam();
  };
